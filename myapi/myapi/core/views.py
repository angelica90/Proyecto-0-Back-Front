...
class event_list(APIView, TemplateView):
    ...
    def get(self, request):
        ...
        events = create_event.objects.filter(event_user=user)
...
class event_detail(APIView):
    ...
    def get(self, request, event_id):
        try:
            ...
            eventos = create_event.objects.filter(owner=user_id, pk=event_id)  # event_id en lugar de id
...
def create_event_f(request):
    ...
    if request.method == 'POST':
        ...
        event = EventForm(event_name=form['event_name'].value(),
...
def events_list(request):
    user = request.user.id
    queryset = create_event.objects.filter(event_user=user)
...
def update_event(request, event_id):
    user = request.user.id
    event = create_event.objects.get(event_user=user, id=event_id)
    form = EventForm(request.POST, instance=event)
    print('Entrar al formulario')
    if form.is_valid():
        print('Entré al formulario')
        form.save()
        return redirect("http://172.24.98.163:8081/api/eventos/")
